...
if NAME == 'Advection':
    path3 = ['adv1_strong/output/000008/result/result.dat']

# time_intervals | tot_time | max_paralpha_iters
eq3 = []
for i in range(len(path3)):
    eq3.append(np.loadtxt(path3[i], delimiter='|', usecols=[1, 2, 6], skiprows=3))
...
for run in range(len(eq3)):
    for subrun in range(no_runs + 1):
        ...
        if eq3[run][subrun, 0] > 1:
            row = int(np.log2(eq3[run][subrun, 0])) - 1
            speedup[row, run] /= eq3[run][subrun, 1]
            its[row, run] = int(eq3[run][subrun, 2])
        else:
            speedup[:, run] *= eq3[run][subrun, 1]
...
proc = range(no_runs)
# nproc = [4, 8, 16, 32, 64]
nproc = [2, 4, 8, 16, 32, 64, 128, 256, 512]
