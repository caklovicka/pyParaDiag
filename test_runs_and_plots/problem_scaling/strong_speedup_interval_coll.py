...
no_runs = len(eq2[0]) - 1
speedup = np.ones((no_runs, len(eq2)))
...
for run in range(len(eq2)):
    for subrun in range(no_runs + 1):
        ...
        if eq2[run][subrun, 1] < 64:
            ...
            speedup[row, run] /= eq2[run][subrun, 3]
            its[row, run] = int(eq2[run][subrun, 4])
        else:
            speedup[:, run] *= seq_run[run + 1]
...
plt.ylim([0, 20])
# plt.show()
plt.savefig('strong_plots/Speedup_' + NAME + '_interval_coll', dpi=300, bbox_inches='tight')