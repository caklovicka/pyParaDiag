...
for k in range(K):
    plt.plot(np.log2(np.array(imex_proc[k])), imex_seq_time[k]/(np.array(imex_time[k]) * np.array(imex_proc[k])), 'v:', color=col[k], linewidth=lw, markersize=mksz)
    if len(newton_proc[k]) > 0:
        plt.plot(np.log2(np.array(newton_proc[k])), newton_seq_time[k]/(np.array(newton_time[k]) * np.array(newton_proc[k])), '^-', color=col[k], linewidth=lw, markersize=mksz)
