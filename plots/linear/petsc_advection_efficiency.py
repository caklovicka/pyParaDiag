...
plt.plot(np.log2(petsc_run[:, 0]), np.array(petsc_run[:, 1]) / np.array(petsc_run[:, 0]), linestyle='-', linewidth=lw, color='silver', marker='D', markersize=marks//2)
...
for i in range(len(eq)):
    ...
    if i > 0:
        plt.plot(np.log2(np.array([12, eq[i][0, 0]])), np.array([petsc_run[4, 1], eq[i][0, 1]]) / np.array([12, eq[i][0, 0]]), linestyle=linst[i], linewidth=lw, color='silver', markersize=marks // 2)

    plt.plot(np.log2(eq[i][:, 0]), np.array(eq[i][:, 1]) / np.array(eq[i][:, 0]), linestyle=linst[i], linewidth=lw, color=col[i])
...
for i in range(len(eq)):
    for nn in range(n):
        m = int(eq[i][nn, 2])
        plt.plot(np.log2(eq[i][nn, 0]), np.array(eq[i][nn, 1]) / np.array(eq[i][nn, 0]), marker="$" + str(m) + "$", markersize=marks, color=col[i])
